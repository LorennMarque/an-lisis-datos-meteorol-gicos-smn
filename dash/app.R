  ...
  ui <- dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        menuItem("Pronóstico", tabName = "pronostico", icon = icon("calendar-alt"))
      )
    ),
    dashboardBody(
      tabItems(
        ...
        tabItem(
          ...
          fluidRow(
            ...
            column(
              ...
              selectInput(
                "plazo_pronostico",
                "Selecciona el plazo de predicción:",
                choices = c(
                  "Próxima semana" = "1w",
                  "Próximo mes" = "1m",
                  "Próximo año" = "1y",
          ...
          plotlyOutput("grafico_pronostico") %>% withSpinner(color = "#0dc5c1")
        )
      )
    )
  )
  ...
  server <- function(input, output) {
    ...
    # Datos filtrados para la predicción
    datos_pronostico <- reactive({
      req(input$provincia_pronostico, input$variable_pronostico)
      datos <- clean_data[provincia == input$provincia_pronostico, .(ds = fecha, y = get(input$variable_pronostico))]
      datos <- datos[!is.na(y), .(y = mean(y, na.rm = TRUE)), by = ds]
      datos <- datos[order(ds)]
      datos
    })
    
    # Generar el pronóstico usando prophet
    prediccion <- reactive({
      datos <- datos_pronostico()
      modelo <- prophet(datos)
      
      # Ajustar el horizonte de predicción según el plazo seleccionado
      horizonte <- switch(input$plazo_pronostico,
                          "1w" = 7,
                          "1m" = 30,
                          "1y" = 365,
                          "2y" = 730,
                          7)  # Valor predeterminado de 7 días si no coincide
      
      future <- make_future_dataframe(modelo, periods = horizonte)
      forecast <- predict(modelo, future)
      list(forecast = forecast, modelo = modelo)
    })
    ...
    output$grafico_pronostico <- renderPlotly({
      ...
      # Filtrar solo las fechas futuras para el pronóstico
      forecast_future <- forecast[forecast$ds > max(datos$ds), ]
      
      plot_ly() %>%
        add_lines(x = datos$ds, y = datos$y, name = "Datos históricos") %>%
        add_lines(x = forecast_future$ds, y = forecast_future$yhat, name = "Pronóstico") %>%
        add_ribbons(x = forecast_future$ds, ymin = forecast_future$yhat_lower, ymax = forecast_future$yhat_upper,
                    name = "Intervalo de confianza", fillcolor = 'rgba(0, 100, 80, 0.2)', line = list(color = 'transparent')) %>%
        layout(
          title = paste("Pronóstico de", names(which(c(
            "temp" = "Temperatura",
            "hum" = "Humedad",
            "pnm" = "Presión",
            "ff" = "Velocidad del viento",
            "precipitacion_mm" = "Precipitación"
          ) == input$variable_pronostico)), "para", input$provincia_pronostico),
          xaxis = list(title = "Fecha"),
          yaxis = list(title = names(which(c(
            "temp" = "Temperatura (°C)",
            "hum" = "Humedad (%)",
