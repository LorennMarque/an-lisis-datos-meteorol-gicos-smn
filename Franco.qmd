...
ggplot(df_prov, aes(x = año, y = precipitaciones_promedio_anual)) +
  geom_line(color = "blue", size = 1) +  # Línea más gruesa
  facet_wrap(~ provincia, ncol = 4, scales = "free_y") +  # Escalas independientes para el eje y
  labs(
    title = "Precipitaciones Promedio Anual por Provincia",
    x = "Año",
    y = "Precipitaciones Promedio (mm)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16),  # Centra el título
...
promedios_evento <- datos_eventos %>%
  ...
  summarize(
    temperatura_promedio = mean(temperatura, na.rm = TRUE),
    humedad_promedio = mean(humedad, na.rm = TRUE),
    presion_promedio = mean(presion_atmosferica, na.rm = TRUE),
